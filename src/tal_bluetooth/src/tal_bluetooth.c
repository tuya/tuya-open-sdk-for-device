    ...
    switch(p_event->type)
    {
        ...
        case TKL_BLE_GAP_EVT_CONNECT: {
            ...
            if(p_event->gap_event.connect.role  == TKL_BLE_ROLE_SERVER) {
                ...
                tal_event.ble_event.connect.peer.char_handle[TAL_COMMON_WRITE_CHAR_INDEX]   = ptkl_ble_service_char[TAL_COMMON_WRITE_CHAR_INDEX].handle;
                tal_event.ble_event.connect.peer.char_handle[TAL_COMMON_NOTIFY_CHAR_INDEX]  = ptkl_ble_service_char[TAL_COMMON_NOTIFY_CHAR_INDEX].handle;
                tal_event.ble_event.connect.peer.char_handle[TAL_COMMON_READ_CHAR_INDEX]    = ptkl_ble_service_char[TAL_COMMON_READ_CHAR_INDEX].handle;
#if ( defined(ENABLE_MATTER_TUYA_BLE_CHAR) && (ENABLE_MATTER_TUYA_BLE_CHAR== 1) )
                tal_event.ble_event.connect.peer.char_handle[TAL_MATTER_TUYA_T1_CHAR_INDEX] = ptkl_ble_service_char[TAL_MATTER_TUYA_T1_CHAR_INDEX].handle;
                tal_event.ble_event.connect.peer.char_handle[TAL_MATTER_TUYA_T2_CHAR_INDEX] = ptkl_ble_service_char[TAL_MATTER_TUYA_T2_CHAR_INDEX].handle;
                tal_event.ble_event.connect.peer.char_handle[TAL_MATTER_TUYA_T3_CHAR_INDEX] = ptkl_ble_service_char[TAL_MATTER_TUYA_T3_CHAR_INDEX].handle;
#endif
...
STATIC VOID tkl_ble_kernel_gatt_event_callback(TKL_BLE_GATT_PARAMS_EVT_T *p_event)
{
    ...
    switch(p_event->type)
    {
        ...
        case TKL_BLE_GATT_EVT_PRIM_SEV_DISCOVERY: {
            ...
            INT_T svc_result = OPRT_INVALID_PARM;
            ...
            if(p_event->result == OPRT_OK) {
                ...
                for(i = 0; i < p_svc_disc->svc_num; i ++) {
                    if(tal_ble_uuid16_convert(&p_svc_disc->services[i].uuid) == TAL_BLE_CMD_SERVICE_UUID_V1
                        ...
                        svc_result = tkl_ble_gattc_all_char_discovery(p_event->conn_handle, p_svc_disc->services[i].start_handle, p_svc_disc->services[i].end_handle);
                        if(svc_result == 0) {
                            return;
                        }
                    }
        ...
        case TKL_BLE_GATT_EVT_CHAR_DISCOVERY: {
            ...
            INT_T char_result = OPRT_INVALID_PARM;
            ...
            if(p_event->result == OPRT_OK) {
                ...
                for(i = 0; i < p_char_disc->char_num; i ++) {
                    ...
                    }else if(tal_ble_uuid16_convert(&p_char_disc->characteristics[i].uuid) == TAL_BLE_CMD_NOTIFY_CHAR_UUID_V1 
                        || tal_ble_uuid16_convert(&p_char_disc->characteristics[i].uuid) == TAL_BLE_CMD_NOTIFY_CHAR_UUID_V2) {

                        tal_ble_peer.char_handle[TAL_COMMON_NOTIFY_CHAR_INDEX] = p_char_disc->characteristics[i].handle;
                    }else if(tal_ble_uuid16_convert(&p_char_disc->characteristics[i].uuid) == TAL_BLE_CMD_READ_CHAR_UUID_V2) {
                ...
                if(tal_ble_peer.char_handle[TAL_COMMON_NOTIFY_CHAR_INDEX] != TKL_BLE_GATT_INVALID_HANDLE) {
                    char_result = tkl_ble_gattc_char_desc_discovery(p_event->conn_handle, tal_ble_peer.char_handle[TAL_COMMON_NOTIFY_CHAR_INDEX], 0xFFFF);
...
/**
 * @brief   [Ble Peripheral] Notify Command characteristic value by characteristic handle
...
OPERATE_RET tal_ble_server_common_send(TAL_BLE_DATA_T *p_data)
{
    ...
    if(tkl_ble_common_connect_handle != TKL_BLE_GATT_INVALID_HANDLE 
        ...
        return tkl_ble_gatts_value_notify(tkl_ble_common_connect_handle, ptkl_ble_service_char[TAL_COMMON_NOTIFY_CHAR_INDEX].handle, 
            p_data->p_data, p_data->len);
    }
...
/**
 * @brief   [Ble Peripheral] Set Read Command characteristic value by characteristic handle
...
OPERATE_RET tal_ble_server_common_read_update(TAL_BLE_DATA_T *p_data)
{
    ...
    if(tkl_ble_common_connect_handle != TKL_BLE_GATT_INVALID_HANDLE 
        ...
        return tkl_ble_gatts_value_set(tkl_ble_common_connect_handle, ptkl_ble_service_char[TAL_COMMON_READ_CHAR_INDEX].handle, 
            p_data->p_data, p_data->len);
    }
...
OPERATE_RET tal_ble_client_common_send(CONST TAL_BLE_PEER_INFO_T peer, TAL_BLE_DATA_T *p_data)
{
    ...
    if(peer.conn_handle != TKL_BLE_GATT_INVALID_HANDLE && peer.char_handle[TAL_COMMON_WRITE_CHAR_INDEX] != TKL_BLE_GATT_INVALID_HANDLE) {
        return tkl_ble_gattc_write_without_rsp(peer.conn_handle, peer.char_handle[TAL_COMMON_WRITE_CHAR_INDEX], p_data->p_data, p_data->len);
    }
...
{
    if(peer.conn_handle != TKL_BLE_GATT_INVALID_HANDLE && peer.char_handle[TAL_COMMON_READ_CHAR_INDEX] != TKL_BLE_GATT_INVALID_HANDLE) {
