...
static os_membuf_t ble_gap_update_entry_mem[
                        OS_MEMPOOL_SIZE((TY_HS_BLE_GAP_MAX_PENDING_CONN_PARAM_UPDATE),
...
#if TY_HS_BLE_ADVERTISE && !(TY_HS_BLE_EXT_ADV)
...
#if TY_HS_BLE_SCAN && !(TY_HS_BLE_EXT_ADV)
...
#if (TY_HS_BLE_ROLE_CENTRAL) && !(TY_HS_BLE_EXT_ADV)
static void
...
{
#if TY_HS_BLE_ADVERTISE
    int i;
#endif
    BLE_HS_DBG_ASSERT(ble_hs_locked_by_cur_task());

#if (TY_HS_BLE_ROLE_CENTRAL) || (TY_HS_BLE_ROLE_OBSERVER)
...
#if (TY_HS_BLE_ROLE_OBSERVER) || TY_HS_BLE_CONNECT
static void
...
#if TY_HS_BLE_ADVERTISE || TY_HS_BLE_CONNECT
static void
...
#if (TY_HS_BLE_ROLE_CENTRAL) || (TY_HS_BLE_ROLE_PERIPHERAL) || (TY_HS_BLE_ROLE_OBSERVER)
static bool
...
ble_gap_master_ticks_until_exp(void)
{
    ...
    ticks = ble_gap_master.exp_os_ticks - tuya_ble_tick_count_get();
    if (ticks > 0) {
...
ble_gap_slave_ticks_until_exp(void)
{
    ...
    ticks = ble_gap_slave[0].exp_os_ticks - tuya_ble_tick_count_get();
    if (ticks > 0) {
        /* Timer not expired yet. */
        return ticks;
    }
...
{
    ble_gap_master.exp_os_ticks = tuya_ble_tick_count_get() + ticks_from_now;
...
{
    ble_gap_slave[0].exp_os_ticks = tuya_ble_tick_count_get() + ticks_from_now;
...
#if (TY_HS_BLE_CONNECT || TY_HS_BLE_SCAN)
/**
...
ble_gap_conn_broken(uint16_t conn_handle, int reason)
{
    ...
    //BLE_HS_LOG(INFO, "connect broken");
    ble_gap_event_listener_call(&event);
...
ble_gap_rx_disconn_complete(const struct ble_hci_ev_disconn_cmp *ev)
{
    ...
    //BLE_HS_LOG(INFO, "rx disconnect complete(0x%02x)", ev->status);
    if (ev->status == 0) {
    ...
    if (conn != NULL) {
        ...
        case BLE_ERR_UNSUPP_REM_FEATURE:
            ...
            if (entry != NULL && !(conn->bhc_flags & BLE_HS_CONN_F_MASTER)) {
                ...
                entry->exp_os_ticks = tuya_ble_tick_count_get() +
                                      tuya_ble_time_ms_to_ticks32(BLE_GAP_UPDATE_TIMEOUT_MS);
            }
            break;
...
#if (TY_HS_BLE_SCAN || TY_HS_BLE_CONNECT)
...
ble_gap_rx_periodic_adv_sync_lost(const struct ble_hci_ev_le_subev_periodic_adv_sync_lost *ev)
{
    ...
    tuya_ble_eventq_remove(ble_hs_evq_get(), &psync->lost_ev);
...
#if (TY_HS_BLE_PERIODIC_ADV_SYNC_TRANSFER)
static int
...
ble_gap_master_timer(void)
{
    ...
    case BLE_GAP_OP_M_CONN:
        ...
        if (rc != 0) {
            ...
            return tuya_ble_time_ms_to_ticks32(BLE_GAP_CANCEL_RETRY_TIMEOUT_MS);
        } else {
...
ble_gap_timer(void)
{
    ...
    min_ticks = MIN_CMP(master_ticks, update_ticks);

#if TY_HS_BLE_ADVERTISE && !(TY_HS_BLE_EXT_ADV)
    min_ticks = MIN_CMP(min_ticks, ble_gap_slave_timer());
#endif
                  ...
                  ble_gap_event_fn *cb, void *cb_arg)
{
    ...
    if (duration_ms != BLE_HS_FOREVER) {
        rc = tuya_ble_time_ms_to_ticks(duration_ms, &duration_ticks);
        if (rc != 0) {
            /* Duration too great. */
            rc = BLE_HS_EINVAL;
            goto done;
...
ble_gap_ext_adv_set_data_validate(uint8_t instance, struct os_mbuf *data)
{
    ...
    if (ble_gap_slave[instance].op == BLE_GAP_OP_S_ADV) {
        if (len > MIN_CMP((TY_HS_BLE_EXT_ADV_MAX_SIZE), 251)) {
            return BLE_HS_EINVAL;
        }
    }
...
#if (TY_HS_BLE_EXT_ADV_MAX_SIZE) <= BLE_HCI_MAX_EXT_ADV_DATA_LEN
    static uint8_t buf[sizeof(struct ble_hci_le_set_ext_adv_data_cp) + \
                       (TY_HS_BLE_EXT_ADV_MAX_SIZE)];
...
{
    /* In that case we always fit all data in single HCI command */
#if (TY_HS_BLE_EXT_ADV_MAX_SIZE) <= BLE_HCI_MAX_PERIODIC_ADV_DATA_LEN
...
static void
ble_gap_npl_sync_lost(struct tuya_ble_event *ev)
{
    struct ble_hs_periodic_sync *psync;
    struct ble_gap_event event;
    ble_gap_event_fn *cb;
    ...
    psync = tuya_ble_event_get_arg(ev);
    cb = psync->cb;
    cb_arg = psync->cb_arg;
                                 ...
                                 ble_gap_event_fn *cb, void *cb_arg)
{
    ...
    tuya_ble_event_set_ev(&psync->lost_ev, ble_gap_npl_sync_lost, psync);
...
ble_gap_periodic_adv_sync_terminate(uint16_t sync_handle)
{
    ...
    if (rc == 0) {
         ...
         */
        tuya_ble_eventq_put(ble_hs_evq_get(), &psync->lost_ev);
    }
                                  ...
                                  ble_gap_event_fn *cb, void *cb_arg)
{
    ...
    if (params) {
        ...
        if (rc) {
            ...
            tuya_ble_event_set_ev(&conn->psync->lost_ev, ble_gap_npl_sync_lost,
                               conn->psync);
        }
    } else {
        if (!conn->psync) {
...
#if (TY_HS_BLE_EXT_ADV) && TY_HS_BLE_SCAN
static int
...
{
#if TY_HS_BLE_SM
    ble_addr_t peer_id_addrs[(TY_HS_BLE_STORE_MAX_BONDS)];
    int num_peers;
    int rc, i;

    rc = ble_store_util_bonded_peers(
            &peer_id_addrs[0], &num_peers, (TY_HS_BLE_STORE_MAX_BONDS));
    if (rc != 0) {
        return rc;
    }
...
{
#if TY_HS_BLE_SM && TY_HS_BLE_CONNECT
    struct ble_gap_event event;
...
{
#if ((TY_HS_BLE_GATT_NOTIFY) || (TY_HS_BLE_GATT_INDICATE)) && TY_HS_BLE_CONNECT
...
static tuya_ble_mutex preempt_done_mutex;
...
ble_gap_preempt_done(void)
{
    ...
    tuya_ble_mutex_lock(preempt_done_mutex, 0xFFFFFFFF);
    ...
    tuya_ble_mutex_unlock(preempt_done_mutex);
