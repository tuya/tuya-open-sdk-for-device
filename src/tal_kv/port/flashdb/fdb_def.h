...
static inline char *fdb_base_name(char *path)
{
    char *offset = path + strlen(path);
...
#define FDB_DEBUG(...)                 FDB_LOG_PREFIX();FDB_PRINT("(%s:%d) ", fdb_base_name(__FILE__), __LINE__);FDB_PRINT(__VA_ARGS__)
#else
#define FDB_DEBUG(...)
#endif
