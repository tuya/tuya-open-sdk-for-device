...
mqtt_client_status_t mqtt_client_init(void* client, const mqtt_client_config_t* config)
{
    ...
    mqtt_status = MQTT_Init( &context->mqclient,
                             &transport,
                             (MQTTGetCurrentTimeFunc_t)tal_system_get_millisecond,
...
mqtt_client_status_t mqtt_client_connect(void* client)
{
    ...
    int ret = tuya_transporter_connect(context->network, context->config.host, context->config.port, context->config.timeout_ms);
    if (OPRT_OK != ret) {
        tuya_transporter_close(context->network);
