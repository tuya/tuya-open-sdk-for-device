...
static os_membuf_t          ble_hci_ram_cmd_buf[OS_MEMPOOL_SIZE((TUYA_BLE_HCI_CMD_BUF_COUNT), TUYA_BLE_HCI_CMD_SZ)]; // change into 2, because of double malloc for seperating the different layers

/* Buffers for HCI events data */
static struct os_mempool    ble_hci_ram_evt_hi_pool;
static os_membuf_t          ble_hci_ram_evt_hi_buf[OS_MEMPOOL_SIZE((TUYA_BLE_HCI_EVT_HI_BUF_COUNT), (TUYA_BLE_HCI_EVT_BUF_SIZE))];

static struct os_mempool    ble_hci_ram_evt_lo_pool;
static os_membuf_t          ble_hci_ram_evt_lo_buf[OS_MEMPOOL_SIZE((TUYA_BLE_HCI_EVT_LO_BUF_COUNT),(TUYA_BLE_HCI_EVT_BUF_SIZE))];
#endif

#define TUYA_BLE_ACL_BLOCK_SIZE  OS_ALIGN(TUYA_BLE_ACL_BUF_SIZE \
                        + sizeof(struct os_mbuf) \
                        + sizeof(struct os_mbuf_pkthdr), OS_ALIGNMENT)
 ...
 */
int
tuya_ble_hci_set_acl_free_cb(os_mempool_put_fn *cb, void *arg)
{
...
void tuya_ble_hci_buf_init(void)
{
     ...
     */
#if defined(TUYA_USE_DYNA_RAM) && (TUYA_USE_DYNA_RAM==0)
    rc = os_mempool_init(&ble_hci_ram_cmd_pool, TUYA_BLE_HCI_CMD_BUF_COUNT, TUYA_BLE_HCI_CMD_SZ, 
                         ble_hci_ram_cmd_buf, "ble_hci_ram_cmd_pool");
    TUYA_HS_ASSERT(rc == 0);
    ...
    rc = os_mempool_init(&ble_hci_ram_acl_pool, TUYA_BLE_ACL_BUF_COUNT, TUYA_BLE_ACL_BLOCK_SIZE,
                             ble_hci_ram_acl_buf, "ble_hci_sock_acl_pool");
    TUYA_HS_ASSERT(rc == 0);
    ...
    rc = os_dynamempool_init(&ble_hci_dyna_ram_evt_pool, (TUYA_BLE_HCI_EVT_HI_BUF_COUNT + TUYA_BLE_HCI_EVT_LO_BUF_COUNT), TUYA_BLE_HCI_EVT_BUF_SIZE, 
                        "ble_hci_dyna_ram_evt_pool", TUYA_USE_DYNA_RAM_FLAG);
    TUYA_HS_ASSERT(rc == 0);
...
#else
    // for ext ram use
    rc = os_mempool_ext_init(&ble_hci_ext_ram_acl_pool, TUYA_BLE_ACL_BUF_COUNT,
                    TUYA_BLE_ACL_BLOCK_SIZE, NULL , "ble_hci_ext_ram_acl_pool");
    TUYA_HS_ASSERT(rc == 0);
...
int tuya_ble_hs_acl_tx(struct os_mbuf *acl_pkt)
{
    ...
    int rc = 0;
    extern OPERATE_RET tkl_hci_acl_packet_send(UCHAR_T *p_buf, USHORT_T buf_len);
        ...
        switch (acl_wait) {
            ...
            case 1:
                if(acl_wait_len <= acl_ll->om_len) {
                    ...
                    memcpy(&acl_data[payload_len + TUYA_BLE_HCI_DATA_HDR_SZ - acl_wait_len], acl_ll->om_data, acl_ll->om_len);
