...
typedef enum {
    TAL_BLE_ADV_TYPE_CS_UNDIR = 0x01,          /**< Connectable and scannable undirected advertising events.
                                                  [Tuya Default Value]*/
    ...
    TAL_BLE_ADV_TYPE_NCNS_UNDIR = 0x05,        /**< Non-connectable non-scannable
                                                  undirected advertising events. */
} TAL_BLE_ADV_TYPE_E;

typedef struct {
    TAL_BLE_ADDR_TYPE_E
    type;            /**< Mac Address Type, Refer to @ TAL_BLE_ADDR_TYPE_E */
    UCHAR_T addr[6]; /**< GAP Address, Address size, 6 bytes */
} TAL_BLE_ADDR_T;

typedef struct {
    USHORT_T adv_interval_min;   /**< Range: 0x0020 to 0x4000  Time = N * 0.625
                                    msec Time Range: 20 ms to 10.24 sec */
    USHORT_T adv_interval_max;   /**< Range: 0x0020 to 0x4000  Time = N * 0.625
                                    msec Time Range: 20 ms to 10.24 sec */
    TAL_BLE_ADV_TYPE_E adv_type; /**< Adv Type, Refer to @ TAL_BLE_ADV_TYPE_E */
    TAL_BLE_ADDR_T direct_addr;  /**< For direct advertising, we need to set
                                    direct addr, otherise, fill NULL */
} TAL_BLE_ADV_PARAMS_T;

typedef struct {
    TAL_BLE_ADDR_T
    peer_addr;                        /**< After Scan Adv, we can get peer mac and mac type */
    TAL_BLE_ADV_DATA_TYPE_E adv_type; /**< Point the advertising type, refer to
                                         @ TAL_BLE_ADV_DATA_TYPE_E */
...
typedef enum {
    TAL_BLE_SCAN_TYPE_ACTIVE,  /**< active scanning, we can scan the respond
                                  data, tuya default value */
...
typedef struct {
    ...
    USHORT_T scan_interval; /**< Range: 0x0004 to 0x4000 Time = N * 0.625 msec
                               Time Range: 2.5 msec to 10.24 sec */
    USHORT_T scan_window;   /**< Range: 0x0004 to 0x4000 Time = N * 0.625 msec
                               Time Range: 2.5 msec to 10.24 seconds */
    USHORT_T timeout;       /**< Scan timeout between 0x0001 and 0xFFFF in seconds,
...
typedef struct {
    ...
    USHORT_T char_handle[6]; /**< Tuya Char Handle, It can reflect any
                                characteristic, Max Characteristic refer to
                                @TKL_BLE_GATT_CHAR_MAX_NUM*/

    TAL_BLE_ADDR_T peer_addr; /**< Reserved, Only Connecetion Handle can stand
                                 for one specific device For Some Reason on
                                 Bluetooth Gateway, we need device's address.*/
} TAL_BLE_PEER_INFO_T;

typedef struct {
    TAL_BLE_PEER_INFO_T
    peer;                  /**< For each report, we will tell the host peer information. this
                              info can be get after being connected*/
    TAL_BLE_DATA_T report; /**< Report Data */
} TAL_BLE_DATA_REPORT_T;

typedef struct {
    USHORT_T min_conn_interval; /**< Range: 0x0006 to 0x0C80, Time = N * 1.25
                                   msec, Time Range: 7.5 msec to 4 seconds. */
    USHORT_T max_conn_interval; /**< Range: 0x0006 to 0x0C80, Time = N * 1.25
                                   msec, Time Range: 7.5 msec to 4 seconds. */
    USHORT_T latency;           /**< Range: 0x0000 to 0x01F3 */
    USHORT_T conn_sup_timeout;  /**< Range: 0x000A to 0x0C80, Time = N * 10 msec,
                                   Time Range: 100 msec to 32 seconds */

    USHORT_T
    connection_timeout; /**< Range: 0 to 5000ms [Tuya Ble Central Use]*/
} TAL_BLE_CONN_PARAMS_T;

typedef struct {
    TAL_BLE_PEER_INFO_T
    peer; /**< Connection handle on which the event occured.*/

    TAL_BLE_CONN_PARAMS_T
    conn_param;     /**< [Optional] After connection is established, we will get
...
typedef struct {
    TAL_BLE_PEER_INFO_T
    peer;           /**< Disconnection handle on which the event occured.*/
    UCHAR_T reason; /**< Disconnection Reason */
} TAL_BLE_DISCONNECT_EVT_T;

typedef struct {
    USHORT_T conn_handle; /**< After ble peripheral send a notification, we will
                             get the result and connection handle */
    USHORT_T char_handle; /**< After ble peripheral send a notification, we will
                             get the result and characteristic handle */

    int32_t result; /**< Notification Result */
} TAL_BLE_NOTIFY_TX_EVT_T;

typedef struct {
    USHORT_T conn_handle; /**< After try to do get_rssi, we will get connection
                             handle */
    CHAR_T rssi;          /**< After try to do get_rssi, we will get rssi */
} TAL_BLE_CONN_RSSI_EVT_T;

typedef struct {
    USHORT_T conn_handle; /**< After try to do exchange mtu, we will get
                             connection handle */
...
typedef struct {
    ...
    TAL_BLE_CONN_PARAMS_T
    conn; /**< Connection Parameters @ TAL_BLE_CONN_PARAMS_T  */
} TAL_BLE_CONN_PARAM_EVT_T;

/**< Define these parameters for ble event */
typedef enum {
    TAL_BLE_ROLE_PERIPERAL = 0x0001, /**< Ble Peripheral Mode. Will Combine with Tuya Ble SDK, and do
                                        server's operations */
    TAL_BLE_ROLE_CENTRAL = 0x0002,   /**< Ble Central Mode. Will Combine with Tuya Bluetooth Gateway
                                        SDK, and do client's operations */
...
typedef enum {
    ...
    TAL_BLE_EVT_CENTRAL_CONNECT_DISCOVERY, /**< Connected as central role,
                                              include mesh gatt, And Complete
    ...
    TAL_BLE_EVT_MTU_REQUEST, /**< MTU exchange request event, For Ble
                                peripheral, we need to do reply*/

    TAL_BLE_EVT_MTU_RSP, /**< MTU exchange respond event, For Ble Central, the
                            ble central has finished the MTU-Request */

    TAL_BLE_EVT_NOTIFY_TX, /**< [Ble peripheral] Transfer data Callback, Will
...
typedef struct {
    ...
    union {
        ...
        TAL_BLE_CONNECT_EVT_T
        connect;                             /**< Receive connect callback, This value can be used with
                                                TAL_BLE_EVT_PERIPHERAL_CONNECT and
                                                TAL_BLE_EVT_CENTRAL_CONNECT_DISCOVERY */
        ...
        TAL_BLE_CONN_PARAM_EVT_T
        conn_param; /**< We will update connect parameters.This value can be
                       used with TAL_BLE_EVT_CONN_PARAM_REQ and
                       TAL_BLE_EVT_CONN_PARAM_UPDATE */
        TAL_BLE_EXCHANGE_MTU_EVT_T
        exchange_mtu;                      /**< This value can be used with
                                              TAL_BLE_EVT_MTU_REQUEST and TAL_BLE_EVT_MTU_RSP */
        TAL_BLE_CONN_RSSI_EVT_T link_rssi; /**< Peer device RSSI value */
        TAL_BLE_NOTIFY_TX_EVT_T
        notify_result;                      /**< [Ble Peripheral] This value can be used with
                                               TAL_BLE_EVT_NOTIFY_TX after Ble Peripheral send a
                                               notification to peer. */
        TAL_BLE_DATA_REPORT_T write_report; /**< This value can be used with
                                               TAL_BLE_EVT_WRITE_REQ */
        TAL_BLE_DATA_REPORT_T data_report;  /**< This value can be used with
                                               TAL_BLE_EVT_NOTIFY_RX */
        TAL_BLE_DATA_REPORT_T
        data_read;                        /**< After we do read attr in central mode, we will get
                                             the callback from bluetooth Kernel */
        TAL_BLE_SUBSCRBE_EVT_T subscribe; /**< used with TAL_BLE_EVT_SUBSCRIBE*/
        TAL_BLE_READ_CHAR_EVT_T
        char_read; /**< read char event, used with TAL_BLE_EVT_READ_CHAR*/
    } ble_event;
