...
#define TAL_WIFI_CHECK_PARM(__EXPRESS)              \
    if (!(__EXPRESS)) return OPRT_INVALID_PARM      \

#define TAL_WIFI_LOCK()                             \
    tal_mutex_lock(s_tal_wifi.mutex)

#define TAL_WIFI_UNLOCK()                           \
    tal_mutex_unlock(s_tal_wifi.mutex)
...
OPERATE_RET tal_wifi_assign_ap_scan(SCHAR_T *ssid, AP_IF_S **ap)
{
    ...
    if (WWM_POWERDOWN == cur_mode) {
        tal_wifi_set_work_mode(WWM_STATION);   //设置为station开始扫描
...
OPERATE_RET tal_wifi_set_country_code(CHAR_T *country_code)
{
    ...
    CHAR_T        *map[] = {"CN", "US", "JP", "EU"};   //数组下标为COUNTRY_CODE_E枚举
...
{
    OPERATE_RET op_ret = OPRT_OK;
    UINT_T dtim = (s_tal_wifi.lps_dtim == 0) ? 1 : s_tal_wifi.lps_dtim;
