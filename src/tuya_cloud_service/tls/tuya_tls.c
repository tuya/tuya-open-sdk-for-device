...
STATIC VOID __tuya_tls_export_keys(void *p_expkey,
...
STATIC INT_T __tuya_tls_socket_recv_cb(VOID *ctx, unsigned char *buf, size_t len)
{
    tuya_mbedtls_context_t* tls_context = (tuya_mbedtls_context_t*)ctx;

    INT_T non_block = tal_net_get_nonblock(tls_context->socket_fd);
    if (non_block == 0) {
    ...
    INT_T activefds_cnt = tal_net_select(tls_context->socket_fd + 1, &readfds, NULL, NULL, tls_context->overtime_s * 1000);
    if (activefds_cnt <= 0) {
    ...
    INT_T rv = tal_net_recv(tls_context->socket_fd, buf, len);
    tal_net_set_block(tls_context->socket_fd, 1 - non_block);

    return rv;
}

STATIC int tuya_tls_ciphersuite_list_PSK[] = {
...
STATIC OPERATE_RET mbedtls_cert_pkey_parse(tuya_tls_hander p_tls_handler)
{
	...
	if (config->client_cert && config->client_pkey) {
        ...
        op_ret = mbedtls_x509_crt_parse(client_cert,
                    (const unsigned char*)tls_context->config.client_cert,
        ...
        op_ret = mbedtls_pk_parse_key(client_pkey,
                    (const unsigned char*)tls_context->config.client_pkey,
...
OPERATE_RET tuya_tls_init(void)
{
    ...
    mbedtls_threading_set_alt(__tuya_tls_mutex_init,
                              __tuya_tls_mutex_free,
                              __tuya_tls_mutex_lock,
                             ...
                             IN INT_T socket_fd, IN INT_T overtime_s)
{
    ...
    op_ret = mbedtls_ssl_config_defaults(p_conf_ctx, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT);
    if (op_ret != 0) {
    ...
    if (tls_context->config.psk_key_size > 0 && tls_context->config.psk_id_size > 0) {
        mbedtls_ssl_conf_psk(p_conf_ctx,
            (const unsigned char *)tls_context->config.psk_key, tls_context->config.psk_key_size,
            (const unsigned char *)tls_context->config.psk_id, tls_context->config.psk_id_size);
    ...
    while ((op_ret = mbedtls_ssl_handshake(p_ssl_ctx)) != 0) {
        if (op_ret == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED) {
            ...
            if (tls_context->config.exception_cb != NULL) {
                ...
                // tls_context->config.exception_cb(TY_TLS_CERT_EXPIRED, whole_hostname);
                //! TODO:
                break;
            }
        }
        if ((op_ret != MBEDTLS_ERR_SSL_WANT_READ  &&
             op_ret != MBEDTLS_ERR_SSL_WANT_WRITE &&
    ...
    if (tls_context->config.f_send && tls_context->config.f_recv) {
        mbedtls_ssl_set_bio(p_ssl_ctx, tls_context->config.user_data, tls_context->config.f_send, tls_context->config.f_recv, NULL);
    }

    PR_DEBUG("TUYA_TLS Success Connect %s:%d Suit:%s", (hostname ? hostname : ""),
             port_num, mbedtls_ssl_get_ciphersuite(p_ssl_ctx));
...
INT_T tuya_tls_write(IN tuya_tls_hander tls_handler, BYTE_T *buf, UINT_T len)
{
    ...
    while (written_len < len) {
        ...
        //PR_ERR("mbedtls_ssl_write returned %d errno %d", ret, tal_net_get_errno());
...
INT_T tuya_tls_read(IN tuya_tls_hander tls_handler, BYTE_T *buf, UINT_T len)
{
    ...
    INT_T value = mbedtls_ssl_read(&(tls_context->ssl_ctx), buf, len);
