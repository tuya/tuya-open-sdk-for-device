                                        ...
                                        uint8_t* output, size_t* olen)
{
    ...
    rt = mbedtls_cipher_auth_decrypt_wrapper(
        ...
        output, olen,
        (unsigned char *)(input + (ilen - AES_GCM128_TAG_LEN)),
        AES_GCM128_TAG_LEN);
    if (rt != OPRT_OK) {
...
int atop_base_request(const atop_base_request_t* request, atop_base_response_t* response)
{
    ...
    uint8_t* body_buffer = tal_malloc(POST_DATA_PREFIX + (request->datalen + AES_GCM128_NONCE_LEN + AES_GCM128_TAG_LEN) * 2 + 1);
    if (NULL == body_buffer) {
