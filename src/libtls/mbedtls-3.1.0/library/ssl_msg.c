...
extern void handle_buffer_resizing( mbedtls_ssl_context *ssl, int downsizing,
...
int mbedtls_ssl_fetch_input( mbedtls_ssl_context *ssl, size_t nb_want )
{
    ...
    {
#if defined(MBEDTLS_SSL_VARIABLE_BUFFER_LENGTH)
        in_buf_len = nb_want + (size_t)( ssl->in_hdr - ssl->in_buf);
        handle_buffer_resizing(ssl,0,in_buf_len, out_buf_len);
...
int mbedtls_ssl_prepare_handshake_record( mbedtls_ssl_context *ssl )
{
    ...
    {
#if defined(MBEDTLS_SUPPORT_HANDSHAKE_FRAGMENT)
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "TLS handshake fragmentation supported" ) );
        ...
        ssl->fragment_buf = mbedtls_calloc( 1, ssl->in_hslen );
        ...
        size_t fragment_offset = ssl->in_msglen;
        memcpy(ssl->fragment_buf,ssl->in_msg,ssl->in_msglen);

        while (fragment_offset < ssl->in_hslen)
        {
            ...
            memcpy(ssl->fragment_buf + fragment_offset,ssl->in_msg,ssl->in_msglen);
            fragment_offset += ssl->in_msglen;
        }

        ssl->in_msg = ssl->fragment_buf;
#else        
