...
static tuya_ble_mutex ble_hs_hci_mutex;
static tuya_ble_sem ble_hs_hci_sem;
...
#if (TY_HS_BLE_HS_PHONY_HCI_ACKS)
...
static void ble_hs_hci_lock(void)
{
    ...
    rc = tuya_ble_mutex_lock(ble_hs_hci_mutex, TUYA_BLE_FOREVER_TIME);
...
static void ble_hs_hci_unlock(void)
{
    ...
    rc = tuya_ble_mutex_unlock(ble_hs_hci_mutex);
    BLE_HS_DBG_ASSERT_EVAL(rc == 0 || rc == OS_NOT_STARTED);
}
...
ble_hs_hci_set_buf_sz(uint16_t pktlen, uint16_t max_pkts)
{
    ...
    PR_INFO("acl buf sz = %d, max pkt = %d, avail pkt = %d\r\n", ble_hs_hci_buf_sz, ble_hs_hci_max_pkts, ble_hs_hci_avail_pkts);
                       ...
                       struct ble_hs_hci_ack *out_ack)
{
    ...
    if (rc == 0) {
        ...
        if (out_ack->bha_opcode != expected_opcode) {
            ...
            BLE_HS_LOG(DEBUG, "out_ack->bha_opcode =0x%02x , expected_opcode =0x%02x", out_ack->bha_opcode , expected_opcode);
        }
    }
...
ble_hs_hci_wait_for_ack(void)
{
    ...
    if (ble_hs_hci_phony_ack_cb == NULL) {
        ...
        ble_hs_hci_ack =
            (void *) tuya_ble_hci_buf_alloc(TUYA_BLE_HCI_BUF_CMD);
        BLE_HS_DBG_ASSERT(ble_hs_hci_ack != NULL);
        rc = ble_hs_hci_phony_ack_cb((void *)ble_hs_hci_ack, 260);
    }
#else
    rc = tuya_ble_sem_pend(&ble_hs_hci_sem,
...
#else
		//BLE_HS_LOG_DEBUG("Opcode = 0x%02x", ble_hs_hci_ack->opcode);
		//BLE_HS_DUMP_LOG(ble_hs_hci_ack->data, ble_hs_hci_ack->length);
#endif
                  ...
                  void *rsp, uint8_t rsp_len)
{
    ...
    if (rc != 0) {
        PR_ERR("ble_hs_hci_cmd_send_buf, rc = %d", rc);
        goto done;
    }

    rc = ble_hs_hci_wait_for_ack();
    if (rc != 0) {
        PR_ERR("ble_hs_hci_wait_for_ack, rc = %d", rc);
    ...
    if (rc != 0) {
        PR_ERR("ble_hs_hci_process_ack, rc = %d", rc);
        ble_hs_sched_reset(rc);
        goto done;
    }
    ...
    if (!rc && (ack.bha_params_len != rsp_len)) {
        BLE_HS_LOG_DEBUG("ack.bha_params_len, rc = %d", rc);
        ble_hs_sched_reset(rc);
        goto done;
    }
    ...
    if (ble_hs_hci_ack != NULL) {
        ...
        tuya_ble_hci_buf_free((uint8_t *) ble_hs_hci_ack);
#else
        tuya_ble_hci_buf_free(TUYA_BLE_HCI_BUF_EVT, (uint8_t *)ble_hs_hci_ack);
#endif
        ble_hs_hci_ack = NULL;
    }
...
{
    if (tuya_ble_sem_get_count(&ble_hs_hci_sem) > 0) {
        /* This ack is unexpected; ignore it. */
#if defined(TUYA_USE_DYNA_RAM) && (TUYA_USE_DYNA_RAM==0)
        tuya_ble_hci_buf_free((uint8_t *) ack_ev);
#else
        tuya_ble_hci_buf_free(TUYA_BLE_HCI_BUF_EVT, (uint8_t *)ack_ev);
#endif
        return;
    }
    BLE_HS_DBG_ASSERT(ble_hs_hci_ack == NULL);
    ...
    tuya_ble_sem_post(&ble_hs_hci_sem);
}

int
ble_hs_hci_rx_evt(uint8_t *hci_ev, uint16_t len)
{
    uint8_t *rx_evt_data;
    ...
    rx_evt_data = tuya_ble_hci_buf_alloc(TUYA_BLE_HCI_BUF_EVT_HI);
    if (!rx_evt_data) {
        PR_ERR("cannot get hci buf");
        tal_system_sleep(5);
        return -1;
    }

    memcpy(rx_evt_data, hci_ev, hci_ev[1] + sizeof(struct ble_hci_ev));
    ev = (void *) rx_evt_data;
    ...
    case BLE_HCI_EVCODE_COMMAND_COMPLETE:
        ...
        if(BLE_HCI_OGF(cmd_complete->opcode) == BLE_HCI_OGF_VENDOR 
            /*&& BLE_HCI_OCF(cmd_complete->opcode) != BLE_HCI_OCF_VS_RD_STATIC_ADDR*/) {
            PR_INFO("[20210623] Not support vendor specific type(0x%02x-0x%02x)",BLE_HCI_OGF(cmd_complete->opcode),BLE_HCI_OCF(cmd_complete->opcode));
#if defined(TUYA_USE_DYNA_RAM) && (TUYA_USE_DYNA_RAM==0)
            tuya_ble_hci_buf_free(rx_evt_data);
#else
            tuya_ble_hci_buf_free(TUYA_BLE_HCI_BUF_EVT, (uint8_t *)rx_evt_data);
#endif
            return 0;
        }
        break;
    ...
    //BLE_HS_LOG_INFO("enqueue = %d, opcde = 0x%02x", enqueue, ev->opcode);
    if (enqueue) {
        ble_hs_enqueue_hci_event(rx_evt_data);
    } else {
        ble_hs_hci_rx_ack(rx_evt_data);
    }
...
ble_hs_hci_init(void)
{
    ...
    rc = tuya_ble_sem_init(&ble_hs_hci_sem, 0);
    BLE_HS_DBG_ASSERT_EVAL(rc == 0);

    rc = tuya_ble_mutex_init(&ble_hs_hci_mutex);
    BLE_HS_DBG_ASSERT_EVAL(rc == 0);
