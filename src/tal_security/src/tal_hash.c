/**
 * @file tal_hash.c
 * @brief This is tal tal_hash file
 * @version 1.0
 * @date 2022-04-22
 *
...
OPERATE_RET tal_sha256_self_test( INT32_T verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        {
            ret = tal_sha256_update_ret( ctx, sha256_test_buf[j],
...
OPERATE_RET tal_sha1_self_test( int verbose )
{
    ...
    for( i = 0; i < 3; i++ )
    {
        ...
        {
            ret = tal_sha1_update_ret( ctx, sha1_test_buf[i],
                                           sha1_test_buflen[i] );
            if( ret != 0 )
                goto fail;
        }
...
static const size_t sha256_mac_test_keylen[7] =
{
...
static const size_t sha256_mac_test_buflen[7] =
{
...
OPERATE_RET tal_sha256_mac_self_test( INT32_T verbose )
{
    ...
    for( i = 0; i < 7; i++ )
    {
        ...
        ret = tal_sha256_mac(sha256_mac_test_key[i],sha256_mac_test_keylen[i], sha256_mac_test_buf[i], sha256_mac_test_buflen[i], sha256_mac );        
        if( ret != 0 )
            goto fail;
...
static const size_t sha1_mac_test_keylen[7] =
{
    20, 4, 20, 25, 20, 80, 80
};
...
static const size_t sha1_mac_test_buflen[7] =
{
    8, 28, 50, 50, 20, 54, 73
};

static const UINT8_T sha1_mac_test_sum[7][20] =
{
...
OPERATE_RET tal_sha1_mac_self_test( INT32_T verbose )
{
    ...
    for( i = 0; i < 7; i++ )
    {
        ...
        ret = tal_sha1_mac(sha1_mac_test_key[i],sha1_mac_test_keylen[i], sha1_mac_test_buf[i], sha1_mac_test_buflen[i], sha1_mac );        
        if( ret != 0 )
            goto fail;
