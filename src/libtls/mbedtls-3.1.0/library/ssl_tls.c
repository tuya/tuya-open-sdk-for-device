...
void handle_buffer_resizing( mbedtls_ssl_context *ssl, int downsizing,
...
#if defined(MBEDTLS_SUPPORT_HANDSHAKE_FRAGMENT)
static int mbedtls_ssl_write_certificate_fragment( mbedtls_ssl_context *ssl, size_t out_hslen)
{
    int ret = MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
    size_t i, n, max_len;
    const mbedtls_x509_crt *crt;

    max_len = mbedtls_ssl_get_max_out_record_payload( ssl );

    ssl->fragment_buf = mbedtls_calloc( 1, out_hslen );
    if( ssl->fragment_buf == NULL )
    {
        return( MBEDTLS_ERR_SSL_ALLOC_FAILED );
    }
    ...
    while( crt != NULL )
    {
        ...
        ssl->fragment_buf[i    ] = MBEDTLS_BYTE_2( n );
        ssl->fragment_buf[i + 1] = MBEDTLS_BYTE_1( n );
        ssl->fragment_buf[i + 2] = MBEDTLS_BYTE_0( n );

        i += 3; memcpy( ssl->fragment_buf + i, crt->raw.p, n );
        i += n; crt = crt->next;
    }

    ssl->fragment_buf[4]  = MBEDTLS_BYTE_2( i - 7 );
    ssl->fragment_buf[5]  = MBEDTLS_BYTE_1( i - 7 );
    ssl->fragment_buf[6]  = MBEDTLS_BYTE_0( i - 7 );

    ssl->out_hslen = out_hslen; /* total length shall be set */

    /* Send the first fragment*/
    memcpy(ssl->out_msg, ssl->fragment_buf, max_len);
    ssl->out_msglen  = max_len;
    ...
    /* Send the following fragment */
    ssl->out_hslen -= max_len;
    while(ssl->out_hslen > 0)
    {
        if(ssl->out_hslen > max_len)
            ssl->out_msglen = max_len;
        else
            ssl->out_msglen = ssl->out_hslen;

        memcpy(ssl->out_msg, ssl->fragment_buf + (out_hslen - ssl->out_hslen), ssl->out_msglen);
...
int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
    ...
    if(i > mbedtls_ssl_get_max_out_record_payload( ssl )) {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "Certificate too large, need fragment %" MBEDTLS_PRINTF_SIZET
                                    " > %" MBEDTLS_PRINTF_SIZET, i, mbedtls_ssl_get_max_out_record_payload( ssl ) ) );
        ret = mbedtls_ssl_write_certificate_fragment( ssl, i);
        /* Free buf anyway */
        if(ssl->fragment_buf){
            mbedtls_free(ssl->fragment_buf);
            ssl->fragment_buf = NULL;
