...
static void ap_app_key_make(uint8_t *app_key)
{
    ...
    app_key[0]  = 'O'; app_key[1]  = 'X'; app_key[2]  = 'L'; app_key[3]  = 'v';
    app_key[4]  = 's'; app_key[5]  = 'l'; app_key[6]  = 'C'; app_key[7]  = 'v';
    app_key[8]  = 'U'; app_key[9]  = 'x'; app_key[10] = 'c'; app_key[11] = 'T';
    app_key[12] = 'P'; app_key[13] = 'L'; app_key[14] = 'G'; app_key[15] = 'O';
...
static int ap_cfg_cmd_patse(ap_netcfg_t *ap, char *data)
{
    ...
    if ((cJSON_GetObjectItem(root, "token") && 
        (cJSON_GetObjectItem(root, "token")->valuestring[0]))) {
        token = cJSON_GetObjectItem(root, "token")->valuestring;
        PR_DEBUG("Parse token:%s", token);
    }

    char *passwd = NULL;
    if ((cJSON_GetObjectItem(root, "passwd") && 
        (cJSON_GetObjectItem(root, "passwd")->valuestring[0]))) {
...
static int ap_get_wifi_list(char* wifi_list, uint16_t wifi_list_size, uint16_t max_cnt)
{
    ...
    for (loop = 0; loop < max_cnt; loop++) {
        ...
        if (offset + 64 > wifi_list_size) { //预留64字节，以防缓存溢出
            break;
        }
        if (!first_ap) {
            offset += sprintf(wifi_list + offset, ",");
        }
        offset += sprintf(wifi_list + offset, "{\"ssid\":\"%s\",\"rssi\":%d,\"sec\":%u}", ap_if[loop].ssid, ap_if[loop].rssi, ap_if[loop].security);
        first_ap = FALSE;
    }
    offset += sprintf(wifi_list + offset, "]}");
...
static int ap_ext_cmd_parse(ap_netcfg_t *ap, CHAR_T *data)
{
    ...
    cJSON *root     = NULL;
    CHAR_T*  buffer = NULL;
    uint16_t buffer_size = 1024 * 4 + 32; //暂定1K缓存
    ...
    } else if (0 == strcmp(reqtype->valuestring, "query_netcfg_stat")) {    //  query_netcfg_stat
