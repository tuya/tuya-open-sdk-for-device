...
static int tuya_ble_host_scan_event(struct ble_gap_event *event, void *arg)
{
    ...
    if(event->type == BLE_GAP_EVENT_DISC) {
        ...
        if(BLE_HCI_ADV_RPT_EVTYPE_ADV_IND == event->disc.event_type || BLE_HCI_ADV_RPT_EVTYPE_NONCONN_IND == event->disc.event_type) {
        ...
        gap_event.gap_event.adv_report.data.p_data = (UCHAR_T *)event->disc.data;
...
static int tuya_ble_host_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    switch (event->type) {
        ...
        case BLE_GAP_EVENT_CONN_UPDATE: {
            ...
            BLE_HS_LOG(NOTICE, "BLE_GAP_EVENT_CONN_UPDATE,0x%x,0x%x,0x%x",desc.conn_itvl, desc.conn_latency, desc.supervision_timeout);
            }break;
...
static int tuya_ble_host_mtu_exchange_callback(uint16_t conn_handle, const struct ble_gatt_error *error, uint16_t mtu, void *arg)
{
    switch (error->status) {
    case 0:
                        ...
                        const struct ble_gatt_svc *service, void *arg)
{
    ...
    UCHAR_T svc_index = p_service->svc_num;
                ...
                const struct ble_gatt_chr *chr, void *arg)
{
    ...
    UCHAR_T char_index = p_char->char_num;
    ...
    case 0:{
        ...
        BLE_HS_LOG_DEBUG("Discovery Characteristics, Value Handle = 0x%04x, UUID Value = 0x%04x", chr->val_handle, chr->uuid.u16.value);
        return 0;
        };
...
static int tuya_ble_desc_disc_callback(uint16_t conn_handle, const struct ble_gatt_error *error, uint16_t chr_val_handle, 
...
static void tuya_ble_stack_event_callback(TKL_BLE_GAP_EVT_TYPE_E type, INT_T result)
{
...
OPERATE_RET tkl_ble_gap_scan_start(TKL_BLE_GAP_SCAN_PARAMS_T CONST *p_scan_params)
{
    ...
    // rc = ble_gap_disc(BLE_HCI_ADV_OWN_ADDR_PUBLIC, BLE_HS_FOREVER, &disc_params,tuya_ble_host_gap_event, &tuya_ble_client);
    if (rc != 0) {
...
OPERATE_RET tkl_ble_gap_connect(TKL_BLE_GAP_ADDR_T CONST *p_peer_addr, TKL_BLE_GAP_SCAN_PARAMS_T CONST *p_scan_params, TKL_BLE_GAP_CONN_PARAMS_T CONST *p_conn_params)
{
    ...
    rc = ble_gap_connect(BLE_HCI_ADV_OWN_ADDR_PUBLIC, &conn_addr, p_conn_params->connection_timeout, &conn_param, tuya_ble_host_gap_event, &tuya_ble_client);
    if (rc != 0) {
        BLE_HS_LOG(ERR, "Error: Failed to connect to device; addr_type=%d addr=0x%02x:0x%02x\n; rc=%d",
                    conn_addr.type, conn_addr.val[0], conn_addr.val[1], rc);
        return rc;
    }
    return OPRT_OK;
}
...
OPERATE_RET tkl_ble_gatts_service_add(TKL_BLE_GATTS_PARAMS_T *p_service)
{
    ...
    for(i = 0; i < p_service->svc_num; i ++) {
        ...
        if(p_service->p_service[i].svc_uuid.uuid_type == TKL_BLE_UUID_TYPE_16) {
            ...
            memcpy((UCHAR_T *)tuya_gatt_svcs[i].uuid + 1, p_service->p_service[i].svc_uuid.uuid.uuid128, (UINT_T)sizeof(ble_uuid128_t));
        }
    ...
    // ble_hs_sched_start();   移到tkl_ble_stack_init接口中
