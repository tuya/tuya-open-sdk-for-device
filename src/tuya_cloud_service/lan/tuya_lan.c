...
#define SERV_PORT_TCP               6668 //设备监听APP TCP连接
#define SERV_PORT_APP_UDP_BCAST     7000 //APP广播,设备监听端口
...
static lan_cfg_t  s_lan_cfg = {
    .client_num                 = CLIENT_LMT,
...
int lan_msg_gcm_encrpt(uint8_t *data, uint32_t len, uint8_t **ec_data, uint32_t *ec_len, uint8_t *key, uint32_t frame_type)
{
    int op_ret = OPRT_OK;
    int olen = 0;
...
static void lan_app_key_make(void)
{
    ...
    app_key2[0]  = 'y'; app_key2[1]  = 'G'; app_key2[2]  = 'A'; app_key2[3]  = 'd';
    app_key2[4]  = 'l'; app_key2[5]  = 'o'; app_key2[6]  = 'p'; app_key2[7]  = 'o';
    app_key2[8]  = 'P'; app_key2[9]  = 'V'; app_key2[10] = 'l'; app_key2[11] = 'd';
    app_key2[12] = 'A'; app_key2[13] = 'B'; app_key2[14] = 'f'; app_key2[15] = 'n';
    tal_md5_ret(app_key2, APP_KEY_LEN, app_key_encode);
    memcpy(app_key2, app_key_encode, APP_KEY_LEN);

    app_key3[0]  = 'W'; app_key3[1]  = 'z'; app_key3[2]  = 'Y'; app_key3[3]  = 'w';
    app_key3[4]  = 'F'; app_key3[5]  = 'x'; app_key3[6]  = 'I'; app_key3[7]  = 'U';
    app_key3[8]  = 'b'; app_key3[9]  = 'i'; app_key3[10] = 'F'; app_key3[11] = 'h';
    app_key3[12] = 'M'; app_key3[13] = 'm'; app_key3[14] = 'N'; app_key3[15] = 'J';
...
/* 将数据前后封装 55aa 后TCP发送 */
static int lan_send(lan_session_t* session, uint32_t fr_num, uint32_t fr_type, uint32_t ret_code, uint8_t *data, uint32_t len, BOOL_T encryption)
{
    int op_ret = OPRT_OK;
    if (session == NULL) {
        PR_ERR("param error");
...
static void lan_make_udp_packets(BYTE_T **out, int *p_olen)
{
    ...
    offset += sprintf(json_buf + offset, "{\"ip\":\"%s\",\"gwId\":\"%s\",\"uuid\":\"%s\"",
                      ip.ip, id, lan->iot_client->config.uuid);
...
int tuya_lan_dp_report(char *dpstr)
{
    ...
    op_ret = tuya_pack_protocol_data(DP_CMD_LAN, (const CHAR_T *)dpstr, PRO_DATA_PUSH, (BYTE_T *)lan->iot_client->activate.localkey, (CHAR_T **)&out, &out_len);
    if (OPRT_OK != op_ret) {
...
static void lan_protocol_process(lan_mgr_t *lan, lan_session_t* session, lpv35_frame_object_t *frame)
{
    ...
    case FRM_TP_NEW_CMD: {
        ...
        char* jsonstr = NULL;
        op_ret = tuya_parse_protocol_data(DP_CMD_LAN, out, out_len, lan->iot_client->activate.localkey, (CHAR_T**)&jsonstr);
        if (OPRT_OK != op_ret) {
    ...
    case FRM_SECURITY_TYPE3:
        ...
        // hmac randA
        tal_sha256_mac(
            (const UINT8_T *)s_lan_mgr->iot_client->activate.localkey,
            strlen(s_lan_mgr->iot_client->activate.localkey),
            session->randA, 
            RAND_LEN,
            session->hmac);
        // make randB
    ...
    case FRM_SECURITY_TYPE5:
        ...
        session->secret_key, &encrypt_olen,
        tag_tmp, LPV35_FRAME_TAG_SIZE);
        if (op_ret != OPRT_OK) {
    ...
    case FRM_QUERY_STAT_NEW: {
        ...
        if (NULL == root) {
            PR_ERR("Json err");
            lan_send(session, frame->sequence, frame->type, 1, (BYTE_T *)"data format error", strlen("data format error"), true);
        ...
        if (NULL == tmp_data) {
            ...
            lan_send(session, frame->sequence, frame->type, 1, (BYTE_T *)"json obj data unvalid", strlen("json obj data unvalid"), true);
            goto FRM_QRY_STAT_ERR;
        }
...
recv_again:
    ...
    while (recv_datalen >= LPV35_FRAME_MINI_SIZE + offset) {
        ...
        // frame_len verify
        uint32_t frame_len = LPV35_FRAME_HEAD_SIZE + sizeof(lpv35_fixed_head_t) + UNI_NTOHL(fixed_head->length) + LPV35_FRAME_TAIL_SIZE;
        if (frame_len > (recv_datalen - offset)) { // recv data not enough buf
            ...
            offset = 0;
            ret = tal_net_recv_nd_size(session->fd,
                                       tmp_recv_buf + recv_datalen,
                                       frame_len + 1 - recv_datalen,
        ...
        if (lan->iot_client->is_activated) {
            if (fr_type == FRM_SECURITY_TYPE3 || fr_type == FRM_SECURITY_TYPE4 || fr_type == FRM_SECURITY_TYPE5) {
                ...
                if (0 == session->secret_key[0]) {
                    //fr_type come first than TYPE3,4,5, wait some packets before close(used in pressure test)
...
BOOL_T __udp_serv_is_in_packet_vaild(BYTE_T* frame_buffer, uint32_t recv_datalen)
{
    ...
    if (memcmp(frame_buffer + LPV35_FRAME_HEAD_SIZE + sizeof(lpv35_fixed_head_t) + length, LPV35_FRAME_TAIL, LPV35_FRAME_TAIL_SIZE) != 0) {
        PR_ERR("udp tail err");
        return false;
    }
...
static void lan_udp_serv_sock_read(int fd)
{
    ...
    lpv35_fixed_head_t* fixed_head = (lpv35_fixed_head_t *)(frame_buffer + LPV35_FRAME_HEAD_SIZE);
    uint32_t frame_len = LPV35_FRAME_HEAD_SIZE + sizeof(lpv35_fixed_head_t) +
                       UNI_NTOHL(fixed_head->length) + LPV35_FRAME_TAIL_SIZE;
    lpv35_frame_object_t frame_out = {0};
    ...
    if ((NULL == cJSON_GetObjectItem(root, "ip")) ||
        (NULL == cJSON_GetObjectItem(root, "from"))) {
...
static int lan_udp_create_serv_socket(void)
{
    ...
    sloop_sock_t udp_sock_info = {
        .sock       = s_lan_mgr->udp_serv_fd,
        .pre_select = NULL,
...
static int lan_tcp_create_serv_socket(lan_mgr_t *lan)
{
    ...
    sloop_sock_t tcp_sock_info = {
        .sock       = lan->tcp_serv_fd,
...
int tuya_lan_register_cb(uint32_t frame_type, lan_cmd_handler_cb handler)
{
    ...
    for (i = 0; i < LAN_CMD_EXT_COUNT; i++) {
        if ((handler == s_lan_cfg.cmd_ext[i].handler) &&
            (frame_type == s_lan_cfg.cmd_ext[i].frame_type)) {
            break;
        }
    }
