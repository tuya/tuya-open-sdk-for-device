...
ble_hs_startup_read_sup_f_tx(void)
{
    ...
    if (!(rsp.features & 0x0000006000000000)) {
        PR_ERR( "Controller doesn't support LE\n");
...
{
    uint16_t le_pktlen = 0;//max len of each packet
    uint16_t max_pkts = 0;
    uint16_t pktlen = 0;
    uint8_t le_max_pkts = 0;//totel num of hci acl data can be stored in data buffers of the controller
    int rc;
    ...
    if (le_pktlen != 0) {
        ...
        PR_INFO("le acl buf sz = %d,max buf = %d \r\n",pktlen,max_pkts);
    } else {
...
int ble_hs_startup_go(void)
{
    ...
    if (rc != 0) {
        PR_ERR( "ble_hs_startup_reset_tx error\n");
        return rc;
    }

    rc = ble_hs_startup_read_local_ver_tx();
    if (rc != 0) {
        PR_ERR( "ble_hs_startup_read_local_ver_tx error\n");
        return rc;
    }
    ...
    if (ble_hs_hci_get_hci_version() < BLE_HCI_VER_BCS_4_0) {
        PR_ERR( "Required controller version is 4.0 (6)\n");
        return BLE_HS_ECONTROLLER;
    }

    BLE_HS_LOG_DEBUG("BLE HCI Version = %d", ble_hs_hci_get_hci_version());
    ...
    if (rc != 0) {
        PR_ERR( "ble_hs_startup_set_evmask_tx error\r\n");
        return rc;
    }

    rc = ble_hs_startup_le_set_evmask_tx();
    if (rc != 0) {
        PR_ERR( "ble_hs_startup_le_set_evmask_tx error\r\n");
        return rc;
    }

    rc = ble_hs_startup_read_buf_sz();
    if (rc != 0) {
        PR_ERR( "ble_hs_startup_read_buf_sz error\r\n");
        return rc;
    }

    rc = ble_hs_startup_le_read_sup_f_tx();
    if (rc != 0) {
        PR_ERR( "ble_hs_startup_le_read_sup_f_tx error\r\n");
        return rc;
    }

    rc = ble_hs_startup_read_bd_addr();
    if (rc != 0) {
        PR_ERR( "ble_hs_startup_read_bd_addr error\r\n");
        return rc;
    }
