...
//TUYA DHCP CLIENT线程运行次数
#define TUYA_DHCP_CLIENT_THREAD_TRY_NUM     5
//TUYA DHCP CLIENT线程while循环等待的时间
#define TUYA_DHCP_CLIENT_THREAD_WAITTIMES   10

//为了让DHCP CLIENT在每次获取IP地址的时候随机化TCP/UDP端口，将tcp.c/udp.c的端口随机函数放在此位置
...
*************************function define********************
***********************************************************/
//DHCP CLIENT每次启动后随机tcp和udp的起始端口
...
#include "lwip/prot/dhcp.h"
            if ((dhcp_state_enum_t)dhcp->state == DHCP_STATE_OFF) {
                ...
                //os_printf(LM_APP, LL_INFO, "\n\rTUYA_LwIP_DHCP_CLIENT: dhcp stop.");
                return DHCP_STOP;
            }
            ...
            if (IPaddress != 0) {
                ...
                if (dhcpc_cb) {
                    dhcpc_cb(IPaddress, ip_addr_get_ip4_u32(netif_ip_netmask4(pnetif)), ip_addr_get_ip4_u32(netif_ip_gw4(pnetif)));
                }
                ...
                if (dhcp->tries > MAX_DHCP_TRIES) {
                    ...
                    iptab[3] = IP_ADDR0;
#endif                    
                    //os_printf(LM_APP, LL_INFO, "\n\rInterface %d DHCP CLIENT timeout", idx);
        ...
        case DHCP_RELEASE_IP:
            //os_printf(LM_APP, LL_INFO, "\n\rTUYA_LwIP_DHCP_CLIENT: Release ip");
            dhcp_release(pnetif);
...
#endif /* LWIP_CONFIG_FAST_DHCP */
{
    ...
    if (NULL == tuya_dhcp_clinet_thrd) {
        ...
        op_ret = tal_thread_create_and_start(&tuya_dhcp_clinet_thrd, NULL, NULL, tuya_dhcp_client_thread, pdhcpc_params, &thread_cfg);
#else
        op_ret = tal_thread_create_and_start(&tuya_dhcp_clinet_thrd, NULL, NULL, tuya_dhcp_client_thread, NULL, &thread_cfg);
#endif /* LWIP_CONFIG_FAST_DHCP */
        if (OPRT_OK != op_ret) {
#if LWIP_CONFIG_FAST_DHCP
