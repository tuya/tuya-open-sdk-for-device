...
//ble_gatts_num_svc_defs is 0 when ble_gatts_start
 ...
 *                                  the caller within the tuya ble stack.
...
ble_gatts_connection_broken(uint16_t conn_handle)
{
    ...
    if (clt_cfgs != NULL) {
        ...
        //BLE_HS_LOG_INFO("os_memblock_put ble_gatts_clt_cfg_pool\n");
...
{
    tuya_ble_hs_free(ble_gatts_svc_defs);
    ble_gatts_svc_defs = NULL;
    ble_gatts_num_svc_defs = 0;
    //BLE_HS_LOG_INFO("Free ble_gatts_free_svc_defs");
}
...
ble_gatts_free_mem(void)
{
    if(ble_gatts_clt_cfg_mem != NULL) {
        tuya_ble_hs_free(ble_gatts_clt_cfg_mem);
        ble_gatts_clt_cfg_mem = NULL;
    }

    if(ble_gatts_svc_entries != NULL) {
        tuya_ble_hs_free(ble_gatts_svc_entries);
        ble_gatts_svc_entries = NULL;
    }
    //BLE_HS_LOG_INFO("Free ble_gatts_clt_cfg_mem ble_gatts_svc_entries");
}
...
ble_gatts_start(void)
{
    ...
    //no need free every time,when malloc success,keep use 
    // ble_gatts_free_mem();
    //no gatt svr add,that is ble_gatts_num_svc_defs = 0  Invalid,need not realloc ram
    if(ble_gatts_num_svc_defs > 0){
    ...
    if ((ble_hs_max_client_configs > 0) && (ble_gatts_clt_cfg_mem == NULL)) {
        ble_gatts_clt_cfg_mem = tuya_ble_hs_malloc(OS_MEMPOOL_BYTES(ble_hs_max_client_configs,sizeof (struct ble_gatts_clt_cfg)));
        // BLE_HS_LOG_INFO("Malloc ble_gatts_clt_cfg_mem");
        if (ble_gatts_clt_cfg_mem == NULL) {
            rc = BLE_HS_ENOMEM;
            goto done;
        }
    ...
    if ((ble_hs_max_services > 0) && (ble_gatts_svc_entries == NULL)) {
        ble_gatts_svc_entries = tuya_ble_hs_malloc(ble_hs_max_services * sizeof *ble_gatts_svc_entries);
        // BLE_HS_LOG_INFO("Malloc ble_gatts_svc_entries");
        if (ble_gatts_svc_entries == NULL) {
            rc = BLE_HS_ENOMEM;
            goto done;
        }
    ...
    for (i = 0; i < ble_gatts_num_svc_defs; i++) {
        rc = ble_gatts_register_svcs(ble_gatts_svc_defs[i],
                                    tuya_ble_hs_cfg.gatts_register_cb,
                                    tuya_ble_hs_cfg.gatts_register_arg);
        if (rc != 0) {
            goto done;
        }
    }
    ...
    //BLE_HS_LOG_INFO("os_mempool_init rc = %d\n", rc);
    if (rc != 0) {
        rc = BLE_HS_EOS;
        goto done;
    }
...
ble_gatts_conn_init(struct ble_gatts_conn *gatts_conn)
{
    if (ble_gatts_num_cfgable_chrs > 0) {
        ...
        //BLE_HS_LOG_DEBUG("os_memblock_get : ble_gatts_clt_cfg_pool");
    ...
    //BLE_HS_LOG_DEBUG("complete the rx conn, init");
    return 0;
}
...
ble_gatts_send_next_indicate(uint16_t conn_handle)
{
    ...
    //BLE_HS_LOG_DEBUG("service debug 1");
    ble_hs_lock();
...
ble_gatts_rx_indicate_ack(uint16_t conn_handle, uint16_t chr_val_handle)
{
    ...
    //BLE_HS_LOG_DEBUG("service debug 2");
...
ble_gatts_chr_updated(uint16_t chr_val_handle)
{
    ...
    //BLE_HS_LOG_DEBUG("service debug :Send notifications and indications to connected devices.");
...
ble_gatts_tx_notifications_one_chr(uint16_t chr_val_handle)
{
    ...
    //BLE_HS_LOG_DEBUG("char one by one");
    for (i = 0; ; i++) {
        ble_hs_lock();
...
ble_gatts_tx_notifications(void)
{
    ...
    // PR_INFO("ble_gatts_num_cfgable_chrs = %d", ble_gatts_num_cfgable_chrs);
    for (i = 0; i < ble_gatts_num_cfgable_chrs; i++) {
...
ble_gatts_bonding_established(uint16_t conn_handle)
{
    ...
    //BLE_HS_LOG_DEBUG("gatt_srv->num_clt_cfgs = %d\n", gatt_srv->num_clt_cfgs);
...
ble_gatts_add_svcs(const struct ble_gatt_svc_def *svcs)
{
    ...
    p =(void *)tal_realloc(ble_gatts_svc_defs,
...
ble_gatts_count_cfg(const struct ble_gatt_svc_def *defs)
{
    ...
    ble_hs_max_client_configs +=
        res.cccds * ((TY_HS_BLE_MAX_CONNECTIONS) + 1);
