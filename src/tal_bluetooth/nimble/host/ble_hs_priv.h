...
#define BLE_HS_MAX_CONNECTIONS (TY_HS_BLE_MAX_CONNECTIONS)
#else
...
#define BLE_HS_ATT_SVR_QUEUED_WRITE_TMO \
    (TY_HS_BLE_ATT_SVR_QUEUED_WRITE_TMO)
#endif
...
int ble_hs_hci_rx_evt(uint8_t *hci_ev, uint16_t len);
...
tuya_ble_eventq *ble_hs_evq_get(void);
...
int ble_mqueue_init(struct ble_mqueue *mq, tuya_ble_event_fn *ev_fn, void *ev_arg);
struct os_mbuf *ble_mqueue_get(struct ble_mqueue *mq);
int ble_mqueue_put(struct ble_mqueue *mq, tuya_ble_eventq *evq, struct os_mbuf *om);

#if (TY_HS_BLE_HS_DEBUG)
    #define BLE_HS_DBG_ASSERT(x) TUYA_HS_ASSERT(x)
    #define BLE_HS_DBG_ASSERT_EVAL(x) TUYA_HS_ASSERT(x)
#else
