             ...
             const struct ble_gatt_dsc *gatt_dsc)
{
    ...
    if (dsc == NULL) {
        /* Out of memory. */
        PR_INFO("cannot get peer_dsc_pool");
                ...
                void *arg)
{
    ...
    peer = arg;
    TUYA_HS_ASSERT(peer->conn_handle == conn_handle);
             ...
             const struct ble_gatt_chr *gatt_chr)
{
    ...
    if (chr == NULL) {
        /* Out of memory. */
        PR_INFO("cannot get peer_chr_pool");
        return BLE_HS_ENOMEM;
    }
...
const struct peer_svc *tuya_ble_svc_find_uuid(const struct peer *peer, const ble_uuid_t *uuid)
{
...
const struct peer_chr *tuya_ble_chr_find_uuid(const struct peer *peer, const ble_uuid_t *svc_uuid,
                   const ble_uuid_t *chr_uuid)
{
    const struct peer_svc *svc;
    const struct peer_chr *chr;

    svc = tuya_ble_svc_find_uuid(peer, svc_uuid);
    if (svc == NULL) {
        return NULL;
    }
...
const struct peer_dsc *tuya_ble_dsc_find_uuid(const struct peer *peer, const ble_uuid_t *svc_uuid,
                   const ble_uuid_t *chr_uuid, const ble_uuid_t *dsc_uuid)
{
    const struct peer_chr *chr;
    const struct peer_dsc *dsc;

    chr = tuya_ble_chr_find_uuid(peer, svc_uuid, chr_uuid);
    if (chr == NULL) {
        return NULL;
    }
...
static int peer_svc_add(struct peer *peer, const struct ble_gatt_svc *gatt_svc)
{
    ...
    if (svc == NULL) {
        /* Out of memory. */
        PR_INFO("cannot get peer_svc_pool");
        return BLE_HS_ENOMEM;
    }
...
int tuya_ble_peer_disc_all(uint16_t conn_handle, peer_disc_fn *disc_cb, void *disc_cb_arg)
{
    struct peer_svc *svc;
    struct peer *peer;
    int rc;
...
int tuya_ble_peer_delete(uint16_t conn_handle)
{
    struct peer_svc *svc;
    struct peer *peer;
    int rc;
...
int tuya_ble_peer_add(uint16_t conn_handle, uint8_t *pmac)
{
    struct peer *peer;
    ...
    if (peer == NULL) {
        /* Out of memory. */
        PR_INFO("cannot get peer pool");
        return BLE_HS_ENOMEM;
    }

    memset(peer, 0, sizeof *peer);
    peer->conn_handle = conn_handle;
    memcpy(peer->peer_mac, pmac, 6);
...
uint16_t tuya_ble_peer_find_handle(uint8_t *pmac)
{
    struct peer *peer;

    SLIST_FOREACH(peer, &peers, next) {
        if(memcmp(peer->peer_mac, pmac, 6) == 0) {
            return peer->conn_handle;
        }
    }
...
struct peer *tuya_ble_peer_find(uint16_t conn_handle)
{
    struct peer *peer;
...
{
    tuya_ble_hs_free(peer_mem);
    peer_mem = NULL;

    tuya_ble_hs_free(peer_svc_mem);
    peer_svc_mem = NULL;

    tuya_ble_hs_free(peer_chr_mem);
    peer_chr_mem = NULL;

    tuya_ble_hs_free(peer_dsc_mem);
    peer_dsc_mem = NULL;
}

int tuya_ble_peer_init(int max_peers, int max_svcs, int max_chrs, int max_dscs)
{
    int rc;
    ...
    peer_mem = tuya_ble_hs_malloc(OS_MEMPOOL_BYTES(max_peers, sizeof (struct peer)));
    if (peer_mem == NULL) {
        rc = BLE_HS_ENOMEM;
        goto err;
    }
    ...
    peer_svc_mem = tuya_ble_hs_malloc(OS_MEMPOOL_BYTES(max_svcs, sizeof (struct peer_svc)));
    ...
    peer_chr_mem = tuya_ble_hs_malloc(OS_MEMPOOL_BYTES(max_chrs, sizeof (struct peer_chr)));
    ...
    peer_dsc_mem = tuya_ble_hs_malloc(OS_MEMPOOL_BYTES(max_dscs, sizeof (struct peer_dsc)));
