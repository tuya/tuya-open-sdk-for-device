...
from kconfiglib import Symbol, Choice, MENU, COMMENT, MenuNode, \
                       BOOL, TRISTATE, STRING, KEY, INT, HEX, \
                       AND, OR, \
...
def _change_node(node):
    ...
    if sc.orig_type in (INT, HEX, STRING, KEY):
        s = sc.str_value
...
def _changeable(node):
    ...
    return sc.orig_type in (STRING, KEY, INT, HEX) or len(sc.assignable) > 1 \
...
def _value_info(sym):
    ...
    return "Value: {}\n".format(
        ...
        if sym.orig_type in (STRING, KEY)
        else sym.str_value)
...
def _value_str(node):
    ...
    if item.orig_type in (STRING, KEY, INT, HEX):
