...
ble_static_assert(sizeof (struct ble_l2cap_hdr) == BLE_L2CAP_HDR_SZ,
               "struct ble_l2cap_hdr must be 4 bytes");

struct os_mempool ble_l2cap_chan_pool;

static os_membuf_t ble_l2cap_chan_mem[
    OS_MEMPOOL_SIZE((TY_HS_BLE_L2CAP_MAX_CHANS) +
                    (TY_HS_BLE_L2CAP_COC_MAX_NUM),
...
#if (TY_HS_BLE_L2CAP_COC_MAX_NUM)
...
ble_l2cap_reconfig(struct ble_l2cap_chan *chans[], uint8_t num, uint16_t new_mtu)
{
    ...
    for (i = 1; i < num; i++) {
        if (conn_handle != chans[i]->conn_handle) {
            BLE_HS_LOG(ERR, "All channels should have same conn handle\n");
            return BLE_HS_EINVAL;
        }
    }
...
{
#if (TY_HS_BLE_L2CAP_JOIN_RX_FRAGS)
    struct os_mbuf *m;
                     ...
                     ble_l2cap_rx_fn **out_rx_cb)
{
    ...
    if (len_diff > 0) {
        ...
        *out_rx_cb = chan->rx_fn;
        ...
        conn->bhc_rx_timeout =
            tuya_ble_tick_count_get() + (TY_HS_BLE_L2CAP_RX_FRAG_TIMEOUT);
