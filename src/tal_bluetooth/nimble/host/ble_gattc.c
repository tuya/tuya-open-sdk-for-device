...
struct ble_gattc_proc {
    ...
    union {
        ...
        struct {
            uint16_t handles[(TY_HS_BLE_GATT_READ_MAX_ATTRS)];
            uint8_t num_handles;
            ble_gatt_attr_fn *cb;
            void *cb_arg;
        } read_mult;
        ...
        struct {
            struct ble_gatt_attr attrs[(TY_HS_BLE_GATT_WRITE_MAX_ATTRS)];
            uint8_t num_attrs;
            uint8_t cur_attr;
            uint16_t length;
...
static os_membuf_t ble_gattc_proc_mem[
    OS_MEMPOOL_SIZE((TY_HS_BLE_GATT_MAX_PROCS),
 ...
 */
static uint32_t ble_gattc_resume_at;
...
ble_gattc_proc_set_exp_timer(struct ble_gattc_proc *proc)
{
    proc->exp_os_ticks = tuya_ble_tick_count_get() +
                         tuya_ble_time_ms_to_ticks32(BLE_GATTC_UNRESPONSIVE_TIMEOUT_MS);
}
...
ble_gattc_proc_set_resume_timer(struct ble_gattc_proc *proc)
{
    ...
    if (ble_gattc_resume_at == 0) {
        ble_gattc_resume_at = tuya_ble_tick_count_get() +
                              tuya_ble_time_ms_to_ticks32((TY_HS_BLE_GATT_RESUME_RATE));
...
ble_gattc_extract_expired(struct ble_gattc_proc_list *dst_list)
{
    ...
    criteria.now = tuya_ble_tick_count_get();
...
ble_gattc_timer(void)
{
    ...
    return MIN_CMP(ticks_until_exp, ticks_until_resume);
}
...
ble_gattc_disc_all_svcs(uint16_t conn_handle, ble_gatt_disc_svc_fn *cb,
                        void *cb_arg)
{
#if !(TY_HS_BLE_GATT_DISC_ALL_SVCS)
...
{
#if !(TY_HS_BLE_GATT_DISC_SVC_UUID)
    return BLE_HS_ENOTSUP;
#endif
...
ble_gattc_find_inc_svcs(uint16_t conn_handle, uint16_t start_handle,
                        ...
                        ble_gatt_disc_svc_fn *cb, void *cb_arg)
{
#if !(TY_HS_BLE_GATT_FIND_INC_SVCS)
    return BLE_HS_ENOTSUP;
#endif
...
ble_gattc_disc_all_chrs(uint16_t conn_handle, uint16_t start_handle,
                        ...
                        void *cb_arg)
{
#if !(TY_HS_BLE_GATT_DISC_ALL_CHRS)
    return BLE_HS_ENOTSUP;
#endif
...
{
#if !(TY_HS_BLE_GATT_DISC_CHR_UUID)
    return BLE_HS_ENOTSUP;
#endif
...
{
#if !(TY_HS_BLE_GATT_DISC_ALL_DSCS)
    return BLE_HS_ENOTSUP;
#endif
                    ...
                    void *cb_arg)
{
    ...
    if (num_handles > (TY_HS_BLE_GATT_READ_MAX_ATTRS)) {
        rc = BLE_HS_EINVAL;
        goto done;
    }
...
ble_gattc_write_no_rsp(uint16_t conn_handle, uint16_t attr_handle,
                       struct os_mbuf *txom)
{
#if !(TY_HS_BLE_GATT_WRITE_NO_RSP)
    return BLE_HS_ENOTSUP;
#endif
...
ble_gattc_write_reliable_tx(struct ble_gattc_proc *proc)
{
    ...
    proc->write_reliable.length =
        MIN_CMP(max_sz, OS_MBUF_PKTLEN(attr->om) - attr->offset);
                         ...
                         ble_gatt_reliable_attr_fn *cb, void *cb_arg)
{
    ...
    if (num_attrs > (TY_HS_BLE_GATT_WRITE_MAX_ATTRS)) {
        rc = BLE_HS_EINVAL;
        goto done;
    }
...
ble_gattc_rx_find_info_idata(uint16_t conn_handle,
                             struct ble_att_find_info_idata *idata)
{
#if !TY_HS_BLE_ATT_CLT_FIND_INFO
    return;
#endif
...
ble_gattc_rx_find_type_value_hinfo(uint16_t conn_handle,
                                   struct ble_att_find_type_value_hinfo *hinfo)
{
#if !TY_HS_BLE_ATT_CLT_FIND_TYPE
    return;
#endif
...
ble_gattc_rx_read_type_adata(uint16_t conn_handle,
                             struct ble_att_read_type_adata *adata)
{
#if !TY_HS_BLE_ATT_CLT_READ_TYPE
    return;
#endif
...
ble_gattc_rx_read_group_type_adata(uint16_t conn_handle,
                                   struct ble_att_read_group_type_adata *adata)
{
#if !TY_HS_BLE_ATT_CLT_READ_GROUP_TYPE
    return;
#endif
...
ble_gattc_rx_read_blob_rsp(uint16_t conn_handle, int status,
                           struct os_mbuf **om)
{
#if !TY_HS_BLE_ATT_CLT_READ_BLOB
    return;
#endif
...
ble_gattc_rx_read_mult_rsp(uint16_t conn_handle, int status,
                           struct os_mbuf **om)
{
#if !TY_HS_BLE_ATT_CLT_READ_MULT
    return;
#endif
...
ble_gattc_rx_prep_write_rsp(uint16_t conn_handle, int status,
                            ...
                            struct os_mbuf **om)
{
#if !TY_HS_BLE_ATT_CLT_PREP_WRITE
    return;
#endif
...
{
#if !TY_HS_BLE_ATT_CLT_EXEC_WRITE
    return;
#endif
...
ble_gattc_init(void)
{
    ...
    if ((TY_HS_BLE_GATT_MAX_PROCS) > 0) {
        rc = os_mempool_init(&ble_gattc_proc_pool,
                             (TY_HS_BLE_GATT_MAX_PROCS),
                             sizeof (struct ble_gattc_proc),
                             ble_gattc_proc_mem,
                             "ble_gattc_proc_pool");
        if (rc != 0) {
