...
static os_membuf_t ble_att_svr_prep_entry_mem[
    OS_MEMPOOL_SIZE((TY_HS_BLE_ATT_SVR_MAX_PREP_ENTRIES),
 ...
 */
int32_t
ble_att_svr_ticks_until_tmo(const struct ble_att_svr_conn *svr, uint32_t now)
{
...
{
#if !(TY_HS_BLE_ATT_SVR_FIND_INFO)
    ...
    //BLE_HS_LOG_INFO("Response find info start_handle = 0x%02x end_handle = 0x%02x", start_handle, end_handle);
...
{
#if !(TY_HS_BLE_ATT_SVR_FIND_TYPE)
    return BLE_HS_ENOTSUP;
#endif
    ...
    //BLE_HS_LOG_DEBUG("req->bavq_start_handle = %d", req->bavq_start_handle);
    //BLE_HS_LOG_DEBUG("start_handle = 0x%04x, end_handle = 0x%04x, req->bavq_attr_type = 0x%04x", start_handle, end_handle, req->bavq_attr_type);
...
{
#if !(TY_HS_BLE_ATT_SVR_READ_TYPE)
    return BLE_HS_ENOTSUP;
#endif
...
ble_att_svr_rx_read(uint16_t conn_handle, struct os_mbuf **rxom)
{
    ...
    rc = ble_att_svr_pullup_req_base(rxom, sizeof(*req), &att_err);
    if (rc != 0) {
        PR_DEBUG("ble_att_svr_pullup_req_base:%d\r\n",rc);
        goto done;
    }
    ...
    *rxom = NULL;
    ...
    if (ble_att_cmd_prepare(BLE_ATT_OP_READ_RSP, 0, txom) == NULL) {
        att_err = BLE_ATT_ERR_INSUFFICIENT_RES;
        rc = BLE_HS_ENOMEM;
        PR_DEBUG("ble_att_cmd_prepare:%d\r\n",rc);
        goto done;
    }

    rc = ble_att_svr_read_handle(conn_handle, err_handle, 0, txom, &att_err);
    if (rc != 0) {
        PR_DEBUG("ble_att_svr_read_handle:%d\r\n",rc);
        goto done;
    }
...
{
#if !(TY_HS_BLE_ATT_SVR_READ_BLOB)
    return BLE_HS_ENOTSUP;
#endif
...
{
#if !(TY_HS_BLE_ATT_SVR_READ_MULT)
    return BLE_HS_ENOTSUP;
#endif
...
{
#if !(TY_HS_BLE_ATT_SVR_READ_GROUP_TYPE)
    return BLE_HS_ENOTSUP;
#endif
    ...
    BLE_HS_LOG_DEBUG("debug rc = %d", rc);
    rc = ble_att_svr_tx_rsp(conn_handle, rc, txom,
...
{
#if !(TY_HS_BLE_ATT_SVR_WRITE_NO_RSP)
    return BLE_HS_ENOTSUP;
#endif
    struct ble_att_write_req *req;
    uint8_t att_err;
                              ...
                              uint8_t *out_att_err)
{
    ...
    if (rc != 0) {
        ...
        *out_att_err = BLE_ATT_ERR_PREPARE_QUEUE_FULL;
        PR_ERR("os_mbuf_appendfrom err:%d\r\n",rc);
        return rc;
    }
    ...
    conn->bhc_att_svr.basc_prep_timeout_at =
        tuya_ble_tick_count_get() + BLE_HS_ATT_SVR_QUEUED_WRITE_TMO;
    // ble_hs_timer_resched();  modify by cc
    extern void ble_hs_dyna_timer_resched(uint16_t time);
    ble_hs_dyna_timer_resched(100);
#endif
...
{
#if !(TY_HS_BLE_ATT_SVR_QUEUED_WRITE)
    return BLE_HS_ENOTSUP;
#endif
    ...
    rc = ble_att_svr_pullup_req_base(rxom, sizeof(*req), &att_err);
    if (rc != 0) {
        PR_ERR("ble_att_svr_pullup_req_base err %d\r\n",rc);
        goto done;
    }
    ...
    if (rc != 0) {
        PR_ERR("ble_att_svr_check_perms err %d\r\n",rc);
        goto done;
    }
    ...
    *rxom = NULL;
    ...
    if (rc != 0) {
        PR_ERR("ble_att_svr_insert_prep_entry err %d",rc);
        goto done;
    }
...
ble_att_svr_free_start_mem(void)
{
    if(ble_att_svr_entry_mem != NULL) {
        tuya_ble_hs_free(ble_att_svr_entry_mem);
        ble_att_svr_entry_mem = NULL;
    }
    //BLE_HS_LOG_INFO("Free ble_att_svr_entry_mem");
}
...
ble_att_svr_start(void)
{
    ...
    if ((ble_hs_max_attrs > 0) && (ble_att_svr_entry_mem == NULL)){
        ble_att_svr_entry_mem = tuya_ble_hs_malloc(OS_MEMPOOL_BYTES(ble_hs_max_attrs,sizeof (struct ble_att_svr_entry)));
        //BLE_HS_LOG_INFO("Malloc ble_att_svr_entry_mem");
        if (ble_att_svr_entry_mem == NULL) {
            rc = BLE_HS_ENOMEM;
            goto err;
        }
...
ble_att_svr_init(void)
{
    ...
    if ((TY_HS_BLE_ATT_SVR_MAX_PREP_ENTRIES) > 0) {
        rc = os_mempool_init(&ble_att_svr_prep_entry_pool,
                             (TY_HS_BLE_ATT_SVR_MAX_PREP_ENTRIES),
                             sizeof (struct ble_att_prep_entry),
