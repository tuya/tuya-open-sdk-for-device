 ...
 * 
 * This file contains the implementation of a software-based I2C (Inter-Integrated Circuit) driver, designed for Tuya IoT devices that lack hardware I2C support or require additional I2C interfaces. 
 * It demonstrates how to perform basic I2C operations such as sending and receiving data over GPIO pins, using bit-banging techniques. 
 * The implementation includes functions for initializing the I2C bus, starting and stopping I2C transactions, reading from and writing to I2C slave devices, and checking data integrity using CRC8. 
 * The example focuses on interfacing with the SHT3x series temperature and humidity sensors, but it can be adapted to communicate with any I2C device.
 * 
 * Key features:
 ...
 * 
 * This implementation is intended for developers working on Tuya IoT projects that require custom I2C communication solutions, offering a flexible and portable approach to I2C device management on platforms without native I2C support.
 * 
 * @note This software I2C driver is designed for educational and development purposes and may not be optimized for all production environments. For critical applications, hardware I2C modules, if available, are recommended.
 * 
 ...
 * @return check result
 */
static int32_t __sht3x_check_crc8(IN const UCHAR_T *data, IN const USHORT_T len, IN const UCHAR_T crc_val)
{
