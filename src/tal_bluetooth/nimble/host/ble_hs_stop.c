...
#define BLE_HOST_STOP_TIMEOUT_MS (TY_HS_BLE_HS_STOP_ON_SHUTDOWN_TIMEOUT)
...
static struct tuya_ble_callout ble_hs_stop_terminate_tmo;
...
ble_hs_stop_done(int status)
{
    ...
    tuya_ble_callout_stop(&ble_hs_stop_terminate_tmo);
...
ble_hs_stop_terminate_all_periodic_sync(void)
{
    ...
    while((psync = ble_hs_periodic_sync_first())){
        ...
        if (rc != 0 && rc != BLE_HS_ENOTCONN) {
            PR_ERR( "failed to terminate periodic sync=0x%04x, rc=%d\n",
                       sync_handle, rc);
            return rc;
        }
    }
...
ble_hs_stop_terminate_conn(struct ble_hs_conn *conn, void *arg)
{
    ...
    if (rc == 0) {
         ...
         */
        PR_ERR( "ble_hs_stop: failed to terminate connection; rc=%d\n", rc);
    }
 ...
 */
static void
ble_hs_stop_terminate_timeout_cb(struct tuya_ble_event *ev)
{
    PR_ERR( "ble_hs_stop_terminate_timeout_cb,"
...
{
    BLE_HS_LOG(DEBUG, "ble_hs_stop_conn_cnt = %d", ble_hs_stop_conn_cnt);
            ...
            ble_hs_stop_fn *fn, void *arg)
{
    ...
    if (ble_hs_stop_conn_cnt > 0) {
        tuya_ble_callout_reset(&ble_hs_stop_terminate_tmo,
                              tuya_ble_time_ms_to_ticks32(BLE_HOST_STOP_TIMEOUT_MS));
    } else {
...
{
    tuya_ble_callout_init(&ble_hs_stop_terminate_tmo, tuya_port_get_dflt_eventq(),
