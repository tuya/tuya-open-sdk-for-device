...
STATIC ble_channel_mgr_t s_ble_channel_mgr = {0};// APP下行通道透传
...
//大数据分包响应APP
VOID_T ble_channle_ack(uint16_t type, uint8_t* data, uint32_t len)
{
    ...
    if (FRM_DOWNLINK_TRANSPARENT_REQ == req->type || FRM_DOWNLINK_TRANSPARENT_SPEC_REQ == req->type) {
        ...
        if (pRawData[1] & 0x02) { // subpacket process
            ...
            if (pRawData[2] == 0) { // first subpacket
                ...
                totalLen = 0;
                receivedLen = 0;
                offset += __extract_packet_len(pRawData + offset, &totalLen); // parse all subpackets data length
                ...
                offset += __extract_packet_len(pRawData + offset, &curSubpacketNo); // parse current subpacket no
                curSubpacketLen = pRawLen - offset;
                memcpy(pBuffer + receivedLen, pRawData + offset, curSubpacketLen);
                receivedLen += curSubpacketLen;
            }

            PR_DEBUG("rece downlink subpacket, curSubpacketNo:%u, curSubpacketLen:%u, receivedLen:%u, totalLen:%u",
