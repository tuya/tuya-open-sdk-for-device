 ...
 * It includes initializing the BLE stack, scanning for BLE advertisements, parsing advertisement data, and managing BLE connections.
 * The example showcases how to set up a BLE central device to interact with BLE peripheral devices, facilitating communication and data exchange in IoT applications.
 * 
 * The implementation covers essential BLE operations such as starting and stopping scans, handling advertisement reports, and initiating connections with peripheral devices. 
 * It also demonstrates how to configure scanning parameters and process advertisement packets to extract useful information such as device addresses, advertisement types, and signal strength (RSSI).
 * 
 * @note This example is designed to provide a basic understanding of operating a BLE central device using Tuya's BLE APIs and should be adapted to fit specific application requirements.
 * 
