                                        ...
                                        unsigned char *tag, size_t tag_len)
{
    ...
    */
    ...
    if ((ret = mbedtls_cipher_setkey(&cipher_ctx, input->key, cipher_info->MBEDTLS_PRIVATE(key_bitlen), MBEDTLS_ENCRYPT)) != 0) {
    ...
    */
    ret = mbedtls_cipher_auth_encrypt_ext(&cipher_ctx,
                                          ...
                                          enc_tmpbuf, input->data_len + tag_len,
                                          olen, tag_len);
                                        ...
                                        unsigned char *tag, size_t tag_len)
{
    ...
    */
    ...
    if (mbedtls_cipher_setkey(&cipher_ctx, input->key, cipher_info->MBEDTLS_PRIVATE(key_bitlen), MBEDTLS_DECRYPT) != 0) {
        PR_ERR("mbedtls_cipher_setkey() returned error\n");
        goto EXIT;
    }
    ...
    */
    ret = mbedtls_cipher_auth_decrypt_ext(&cipher_ctx,
                                          input->nonce, input->nonce_len,
                                          input->ad, input->ad_len,
                                          dec_tmpbuf, input->data_len + tag_len,
                                          output, input->data_len,
                                          olen, tag_len);
EXIT:
    mbedtls_cipher_free(&cipher_ctx);
    if (NULL != dec_tmpbuf) {
